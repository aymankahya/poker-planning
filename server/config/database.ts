...
const dbName =
  process.env.ENV === "prod" ? process.env.DB_PROD : process.env.DB_DEV;
...
const connectDB = async () => {
  try {
    ...
    console.log("\x1b[32mConnection to MongoDB established successfully");
  } catch (err) {
    // Emit error message in case of connection failure
    console.error("\x1b[31mConnection to MongoDB failed");
  }
};
